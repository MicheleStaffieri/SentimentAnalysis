        ...
        for feeling in feeling_list:
            ...
            for file_resource in os.listdir(RES_PATH + feeling):
                with open(RES_PATH + feeling + "/" + file_resource, 'r') as file:
                    ...
                    printresult.write(f'Dati\n')
                    printresult.write(f'Numero parole risorsa lessicale: {len(res_words)}\n')
                    printresult.write(f"Numero parole nell'intersezione tra tweets e risorse: {len(self.intersection[feeling][resource_name])}\n")
                    printresult.write(f'Numero parole tweets: {len(self.tweets_table[feeling])}\n')
